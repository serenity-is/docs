...
Let's say we have a Person entity with Firstname and Surname properties:
...
So, for example, if a person has `FirstName`, `MiddleName`, and `Surname` properties and you use `CONCAT` for them to build the full name, as most people don't have a middle name, you'll end up with a `null` FullName for some servers.
