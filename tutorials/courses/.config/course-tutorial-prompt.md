...
As you don't have access to Visual Studio at the moment, you may create a project from the command line using `dotnet new serene -n CourseTutorial_yyMMdd_hhmm` where yyMMdd_hhmm should be replaced based on current date/time. Run this command under p:\internal\tutorial folder. This will create a web project under folder p:\internal\tutorial\CourseTutorial_yyMMdd_hhmm\CourseTutorial_yyMMdd_hhmm.Web. Apply further edits and run project in this folder. Note that as unlike the tutorial, the project won't be named CourseTutorial_yyMMdd_hhmm so when checking things like database adjust the names accordingly, e.g. CourseTutorial_Default_v1 database will be named CourseTutorial_yyMMdd_hhmm_Default_v1.
...
- Adjust namespaces in every code block: `CourseTutorial.` → the actual project namespace (`CourseTutorial_<timestamp>.`). Database names follow the project name too.
...
- MasterDetailRelation ColumnsType from a Row must be qualified: `typeof(Columns.CourseCastColumns)`.
...
Currently the tutorial is one file, but may change in the future.
