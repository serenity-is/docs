# PageAuthorizeAttribute constructor (1 of 6)

Initializes a new instance of the [`PageAuthorizeAttribute`](../PageAuthorizeAttribute.md) class.

```csharp
public PageAuthorizeAttribute()
```

## See Also

* class [PageAuthorizeAttribute](../PageAuthorizeAttribute.md)

---

# PageAuthorizeAttribute constructor (2 of 6)

Initializes an instance of the [`PageAuthorizeAttribute`](../PageAuthorizeAttribute.md) class with the passed permission key.

```csharp
public PageAuthorizeAttribute(object permission)
```

| parameter | description |
| --- | --- |
| permission | The permission key. |

## See Also

* class [PageAuthorizeAttribute](../PageAuthorizeAttribute.md)

---

# PageAuthorizeAttribute constructor (3 of 6)

Initializes an instance of the [`PageAuthorizeAttribute`](../PageAuthorizeAttribute.md) class, while reading the permission key from the NavigationPermissionAttribute or ReadPermissionAttribute of the source type.

```csharp
public PageAuthorizeAttribute(Type sourceType)
```

| parameter | description |
| --- | --- |
| sourceType | The source type. |

## See Also

* class [PageAuthorizeAttribute](../PageAuthorizeAttribute.md)

---

# PageAuthorizeAttribute constructor (4 of 6)

Initializes an instance of the [`PageAuthorizeAttribute`](../PageAuthorizeAttribute.md) class with a permission key generated by joining the passed permissions with a colon (`:`), e.g. `module:permission`.

```csharp
public PageAuthorizeAttribute(object module, object permission)
```

| parameter | description |
| --- | --- |
| module | The module. |
| permission | The permission key. |

## See Also

* class [PageAuthorizeAttribute](../PageAuthorizeAttribute.md)

---

# PageAuthorizeAttribute constructor (5 of 6)

Initializes a new instance of the [`PageAuthorizeAttribute`](../PageAuthorizeAttribute.md) class.

```csharp
protected PageAuthorizeAttribute(Type sourceType, params Type[] attributeTypes)
```

| parameter | description |
| --- | --- |
| sourceType | The source type. |
| attributeTypes | The attribute types. |

## Exceptions

| exception | condition |
| --- | --- |
| ArgumentNullException | Source type or attribute types is `null`. |
| ArgumentOutOfRangeException | One of the attribute types is not a subclass of PermissionAttributeBase. |

## See Also

* class [PageAuthorizeAttribute](../PageAuthorizeAttribute.md)

---

# PageAuthorizeAttribute constructor (6 of 6)

Initializes an instance of the [`PageAuthorizeAttribute`](../PageAuthorizeAttribute.md) class with a permission key generated by joining the passed permissions with a colon (`:`), e.g. `module:submodule:permission`.

```csharp
public PageAuthorizeAttribute(object module, object submodule, object permission)
```

| parameter | description |
| --- | --- |
| module | The module. |
| submodule | The submodule. |
| permission | The permission key. |

## See Also

* class [PageAuthorizeAttribute](../PageAuthorizeAttribute.md)