...
Fortunately, there's a "GenreDialog" class defined in the "Modules/MovieDB/Genre/GenreDialog.tsx" file. When Sergen generates it, the class is registered with the module namespace followed by a dot, and Serenity automatically appends the class name (a type full name can't end with a dot):
...
`nsMovieDB` is a namespace constant generated by `dotnet sergen servertypings` (in `Modules/ServerTypes/Namespaces.ts`) whose value is `"MovieTutorial.MovieDB."`. Prefer these namespace constants over hardcoding full names like `"MovieTutorial.MovieDB.GenreDialog"`, as they automatically stay in sync with the project namespace.
...
> The "correct full name" mentioned in that message is satisfied by registering with the module namespace (with or without the trailing dot) — e.g. `this.registerClass(nsMovieDB)` or `this.registerClass("MovieTutorial.MovieDB.")` — since Serenity appends the class name automatically. You don't need to write out the complete `"MovieTutorial.MovieDB.GenreDialog"` name by hand.
