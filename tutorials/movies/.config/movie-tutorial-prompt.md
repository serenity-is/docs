...
As you don't have access to Visual Studio at the moment, you may create a project from the command line. To keep screenshots clean (the project name appears in the browser title bar, the login page, and the sidebar), always use the SAME project name `MovieTutorial` for every run, but place each run in its own time-based folder so runs don't collide.

1. Pick a run timestamp: `yyMMdd_hhmm` (e.g. `260802_1524`).
2. Create the project under `p:\internal\tutorial`:
   ```
   Set-Location 'p:\internal\tutorial'
   dotnet new serene -n MovieTutorial -o MovieTutorial_<yyMMdd_hhmm>
   ```
   This creates a web project at `p:\internal\tutorial\MovieTutorial_<yyMMdd_hhmm>\MovieTutorial.Web` (folder is time-based, project is always named `MovieTutorial`).
3. Because the project is always named `MovieTutorial`, its default database names (`MovieTutorial_Default_v1`, `MovieTutorial_Northwind_v1`) can clash with LocalDB databases left over from earlier runs. Before launching, edit `appsettings.json` in the project and time-suffix the database names, e.g.:
   - `Database=MovieTutorial_<yyMMdd_hhmm>_Default_v1`
   - `Database=MovieTutorial_<yyMMdd_hhmm>_Northwind_v1`
4. Apply further edits and run the project from the `MovieTutorial.Web` folder.

Since the project is named `MovieTutorial`, the namespace already matches the tutorial's code blocks — do NOT adjust namespaces. When checking the database, use the time-suffixed name, e.g. `MovieTutorial_<yyMMdd_hhmm>_Default_v1`.
...
- Verify DB with: `sqlcmd -S "(localdb)\MsSqlLocalDB" -d MovieTutorial_<yyMMdd_hhmm>_Default_v1 -Q "..." -W` (use the time-suffixed DB name you set in appsettings.json)
...
- Generated clienttype attribute FILES are named after the full namespace: a formatter registered under a module (e.g. `nsMovieDB + "X"` → `MovieTutorial.MovieDB.X`) generates `Imports/ClientTypes/MovieDB.XAttribute.cs` (module-prefixed filename), not `XAttribute.cs`. Search for the module-prefixed filename when looking for a generated attribute.
