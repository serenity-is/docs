...
3. XYZEndpoint injects the auto-registered list handler via `[FromServices]` and calls its `List` method with the retrieved ListRequest object.
4. ListRequestHandler builds up a dynamic SQL query, based on the ListRequest, metadata in its entity type (Row) and other information and executes it.
5. ListRequestHandler returns a ListResponse with Entities member that contains rows to be returned.
