...
If you are using `StartSharp` and have a reference to the `Serenity.Pro.Coder` package, it is possible to simplify the above row declaration by using a `[GenerateFields]` attribute:
...
> You still have to write getter / setter code for properties, as C# does not yet allow partial properties.
