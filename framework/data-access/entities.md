...
## StartSharp and Row Fields Source Generator

If you are using `StartSharp` and have a reference to the `Serenity.Pro.Coder` package, it is possible to simplify the above row declaration by using a `[GenerateFields]` attribute. Since C# 13 supports partial properties, you declare the properties without getter/setter bodies:
...
partial class SimpleRow : Row<SimpleRow.RowFields>
{
    ...
    {
        get { if (age is null && ((IRow)this).TrackWithChecks) OnFieldGet(fields.Age); return age; }
        set { age = value; if (((IRow)this).TrackAssignments) OnFieldSet(fields.Age); }
    }
...
This is also more performant, as private fields are used to store field values instead of an internal dictionary.

> The generator fills in the getter/setter bodies (with assignment tracking and `TrackWithChecks` support), so you only write the partial property declaration. C# 13+ is required for this.

It is also possible to use a nested `RowTemplate` class. Please note that defining a RowTemplate is not recommended since 6.8.1, as it might sometimes cause unexpected issues related to attributes and their parameters, as the source generator has to copy them from the template to the row class, and some of those attributes might be generated by another source generator like `ClientTypesSourceGenerator` on the fly.
