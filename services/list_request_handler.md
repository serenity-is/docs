...
The `ListRequestHandlerAsync` is the base class that handles List requests originating from client side, e.g. from grids.

> From Serenity 10.5.0, Sergen generates **asynchronous** handlers (`ListRequestHandlerAsync`) and endpoint actions by default. The synchronous [`ListRequestHandler`](../api/dotnet/Serenity.Net.Services/Serenity.Services/ListRequestHandler-1.md) is kept for backward compatibility and is marked **obsolete** — it will be deprecated in a future version. New code — and code generated by Sergen — should use the async variants.
...
3. XYZEndpoint injects the auto-registered list handler via `[FromServices]` and calls its `ListAsync` method with the retrieved ListRequest object and a `CancellationToken`.
4. ListRequestHandlerAsync builds up a dynamic SQL query, based on the ListRequest, metadata in its entity type (Row) and other information and executes it asynchronously.
5. ListRequestHandlerAsync returns a ListResponse with Entities member that contains rows to be returned.
6. XYZEndpoint receives this ListResponse, returns it from action.
7. ListResponse is serialized to JSON, sent back to client
8. Grid receives entities, updates its displayed rows and other parts like paging status.
...
When calling a List handler server side to sort by Country then City descending, you might do it like this:

```cs
var handler = new CustomerListHandler(context);
await handler.ListAsync(connection, new ListRequest
{
    Sort = new[] { 
        new SortBy("Country"),
...
```cs
// CustomerService is the client-side proxy for CustomerEndpoint,
// generated by the service client types (server typings)
CustomerService.List(new ListRequest
{
...
```cs
CustomerService.List(connection, {
    EqualityFilter = new()
    {
        ["Country"] = "Germany"
    }
...
protected override setViewParams() {
    ...
    // the view is the data source for the grid (RemoteView);
    // for an EntityGrid, this.view.params is a ListRequest
    const request = this.view.params as ListRequest;
    const fld = ProductRow.Fields;

    // combine with any existing criteria (e.g. set by the edit filter
    // dialog) instead of replacing it, so those filters are not cleared.
...
setViewParams is a good place to set this parameter (and some others):
...
The generated list handler derives from `ListRequestHandlerAsync<TRow>`, which derives from the fully generic [ListRequestHandlerAsync&lt;TRow, TListRequest, TListResponse&gt;](../api/dotnet/Serenity.Net.Services/Serenity.Services/ListRequestHandlerAsync-3.md):

```cs
public class ListRequestHandlerAsync<TRow>(IRequestContext context)
    : ListRequestHandlerAsync<TRow, ListRequest, ListResponse<TRow>>(context), IListHandlerAsync<TRow>
    where TRow : class, IRow, new()
{
}
```

The extra generic arguments let you customize the request and response types. For example, `ListRequestHandlerAsync<TRow, TListRequest, ListResponse<TRow>>` uses a custom list request with the standard `ListResponse<TRow>`.

The sync base classes ([`ListRequestHandler<TRow>`](../api/dotnet/Serenity.Net.Services/Serenity.Services/ListRequestHandler-1.md), [`IListHandler<TRow>`](../api/dotnet/Serenity.Net.Services/Serenity.Services/IListHandler-1.md), ...) are kept for backward compatibility and are marked obsolete.
...
[`IListRequestProcessor`](../api/dotnet/Serenity.Net.Services/Serenity.Services/IListRequestProcessor.md) is the abstraction with a `Process(connection, request)` method, used when a handler needs to run a list query without going through the endpoint. Its async counterpart is [`IListRequestProcessorAsync`](../api/dotnet/Serenity.Net.Services/Serenity.Services/IListRequestProcessorAsync.md) with `ProcessAsync(connection, request, cancellationToken)`. `ListRequestHandlerAsync<TRow>` implements the async variant.

### Overriding list behavior

When you override a lifecycle method on a list handler, use the async variant and call the base implementation first. For example, to add an extra filter:
...
The overridable methods are `ValidateRequestAsync`, `PrepareQueryAsync`, `ApplyFiltersAsync`, `OnBeforeExecuteQueryAsync`, `OnAfterExecuteQueryAsync` and `OnReturnAsync`.
